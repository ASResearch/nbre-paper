...
\begin{itemize}
\item  The default journal template style.
\item  Used by JOCCH and TAP.
\item  Used by TOG.
\end{itemize}
...
In this section, we first define blockchain protocol and version, and describe
blockchain abstraction based on the definition. We then introduce Blockchain
...
both permissionless and permissioned, blockchain. Generally, BRE is not
going to run independently, but will interact with blockchain when manipulating
upgrading or provide service when receiving blockchain requests.

Upgrading blockchain, actually, is upgrading blockchain protocol. Specifically,
...
In view of above-mentioned description, blockchain peer, blockchain protocol
and blockchan peer state is as follows:
\begin{itemize}
  \item $n_i$ is blockchain peer with identity $i=1,2,...$
  \item $p_j$ is blockchain protocol with version $j=1,2,...$
  \item $S_{m}^{j}=\{p_1,p_2,..,p_j,..,p_m\}$ is blockchain peer state, where
    the peer holds a set of $m$ kinds of historical protocols, and the peer
    is running with protocol $p_j$ currently.
\end{itemize}

Thus, we further abstract blockchain system along with blockchain peer state.
...
\subsection{Parse Phase}
\subsection{Compilation Phase}
\subsection{Upgrade Phase}
