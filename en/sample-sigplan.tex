...
Blockchain is a distributed database that each peer holds the entire
information of the database. The information of each one is a growing list of
records, called blocks, that are linked using cryptography.
Each block contains a cryptographic hash of the previous block and transaction
data \cite{blockchainwikidef}.
Underlying consensus protocol ensures blockchain peers share the same state.

Based on the basic concept of blockchain (block, transaction and consensus
protocol), arguments are proposed because of the different requirements in
different scenarios of blockchain.
Or with the development of blockchain technology and deeper thinking of
blockchain, ideas on blockchain optimization are raised.
...
upgrade or keep as it is, making it impossible to enforce peers upgrade.
